...
def main():
    """
    Converts audio files within a directory to a specified format using ffmpeg and multiprocessing.

    The input and output directories are specified through command line arguments.
    The script uses multiprocessing to convert multiple audio files concurrently.
    ...
    audio_files = [file for file in os.listdir(input_folder) if file.endswith(('.mp3', '.wav', '.ogg', '.flac', '.aac', '.wma'))]
